...
Vercord uses Upstash Redis for rate limiting. Add the credentials and it's enabled. No credentials = no rate limiting.

<Callout type="error">
  **Skipping this step?** Your webhook endpoint will be unprotected against spam, abuse, and DDoS. Only skip if you have alternative protection (Cloudflare, Vercel Firewall, etc.).
</Callout>
